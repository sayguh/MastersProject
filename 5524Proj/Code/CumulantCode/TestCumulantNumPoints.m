function pointsMatrix = TestCumulantNumPoints(SNR, dataSet)
...
for ii = 1: length(pointVector)
	...
	for i = 1:25
		...
		(CumulantClassifier(phaseOffset*dataSet.BPSKdata(i,1:pointVector(ii)) + noiseMatrix(i,1:pointVector(ii))) == 2);

		QAMClassification(ii) += ...
		(CumulantClassifier(phaseOffset*dataSet.QAMdata(i,1:pointVector(ii)) + noiseMatrix(i,1:pointVector(ii))) == 4);

		QAM16Classification(ii) += ...
		(CumulantClassifier(phaseOffset*dataSet.QAM16data(i,1:pointVector(ii)) + noiseMatrix(i,1:pointVector(ii))) == 16);

		QAM64Classification(ii) += ...
		(CumulantClassifier(phaseOffset*dataSet.QAM64data(i,1:pointVector(ii)) + noiseMatrix(i,1:pointVector(ii))) == 64);
